...
namespace ServiceTool.Legacy.Dongle
{
    public class PortSwitch
    {
        ...
        public event EventHandler<REM.MessageRDO> OnMesageReceived;
        ...
        internal async Task InitPort()
        {
            ...
            await data.Send(Constants.DongleCommandMode);
            await Task.Delay(50);
            //Send break command
            Log("Send Break Command...");
            await command.Send(Constants.DongleBreakCMD);
            await Task.Delay(1000);
            //Send Carriage return
            Log("Carriage Return...");
            await data.Send(Constants.CarriageReturn);
            await Task.Delay(200);
            //SendMMdata
            Log("MM...");
            await data.Send(Constants.DoubleEm);
            //SVT logon
            Log("SVT Logon...");
            await data.Send(Constants.CarriageReturn);
            //SVT escape sequence
            Log("SVT Escape Sequence...");
            await data.Send(Constants.EscLogon);
        ...
        internal async Task DefaultBaud()
        {
            ...
            await command.Send(Constants.DongleDefaultBaud);
            await command.Send(Constants.DongleUpdateConfig);
            await command.Send(Constants.DongleGetBaudRate);
        }
        ...
        internal async Task GECBBaud()
        {
            ...
            await command.Send(Constants.DongleBaud38K);
            await command.Send(Constants.DongleUpdateConfig);
            await command.Send(Constants.DongleGetBaudRate);
            await Task.Delay(200);
            await command.Send(Constants.DongleBaud57K);
            await command.Send(Constants.DongleUpdateConfig);
            await command.Send(Constants.DongleGetBaudRate);
        ...
        internal async Task SwapBackToSVT()
        {
            ...
            Log("SVT Logon...");
            await data.Send(Constants.CarriageReturn);
            await command.Send(Constants.DongleReboot);
        }
        ...
        {
            Console.WriteLine(REM.Helper.ByteArrayToString(rawData));
            ...
            foreach (byte i in rawData)
            {
                switch (i)
                {
                    ...
                    case 0x62:
                        ...
                        OnPortswapError?.Invoke(this, Constants.SVTLogonError);
                        break;
                    case 0x68:
                        OnPortswapError?.Invoke(this, Constants.SVTLogonError);
        ...
        /// </summary>
        /// <param name="msg">Message.</param>
        internal void MessageForUI(REM.MessageRDO msg)
        {
            if (dbc != null)
            {
        ...
        internal void DBCErrorHandler(int status)
        {
            switch (status)
            {
                ...
                case (int) ErrorStatus.SVT_CONNECTED:
                    OnPortswapError?.Invoke(this, Constants.SVTConnected);
                    break;
                case (int) ErrorStatus.SVT_TIMEOUT:
                case (int) ErrorStatus.SVT_NO_RESPONSE:
                    OnPortswapError?.Invoke(this, Constants.SVTNoResponse);
                    break;
            }
        }
